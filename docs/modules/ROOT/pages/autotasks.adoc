...
Autotask secrets are key-value case-sensitive pairs of strings, that can be accessed from any autotask in the `event.secrets` object. You can define as many secrets as you need to be used by your autotasks. Secrets are shared across all your autotasks, and not specific to a single one.
...
Secrets are encrypted and stored in a secure vault, only decrypted for injection in your autotasks runs. Once written, a secret can only be deleted or overwritten from the user interface, but not read. However, an autotask may still log the value of a secret, accidentally leaking it.
...
All `defender-client` packages are coded in typescript and are packaged with their type declarations. You can use the following type definition for your Autotasks handler function:
...
type RelayerParams = ApiRelayerParams | AutotaskRelayerParams;
...
We are working to better connect Autotasks with other parts of the system, such as the Notification channels used from Sentinels so you can send emails or messages easily, or the Address Book used in Admin so you can rely on it as a registry. Let us know if you have anything in mind!
