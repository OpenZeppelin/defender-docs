...
NOTE: The code bundle must not exceed 50MB in size, and you must always include an `index.js` at the root of the zip file to act as entrypoint.
...
We are working on a secrets management solution so you can secure any keys you need for securely accessing external authenticated resources from the Autotask code, and we are also working on simplifying sending notifications from Autotasks. Stay tuned!
