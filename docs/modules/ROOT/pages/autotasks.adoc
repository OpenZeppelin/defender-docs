...
The key-value store allows you to get, put, and delete keys. All keys and values are strings, keys are limited to 1kb and values to 300kb. You can store up to 1000, 3000, or 10000 key-value pairs in total, depending if you are on the free, individual, or paid plan.
...
WARNING: All items expire 90 days after their last update. If you need a long-lived data store, consider setting up an external database and use Autotask secrets to store the credentials for connecting to it.
