...
Autotasks are executed in a https://nodejs.org/dist/latest-v12.x/docs/api/[node 12 runtime] with 256mb RAM and a 5-minute timeout. Code snippets are restricted to be smaller than 5MB in size (after being compressed and base64-encoded). For ease-of-use, a set of common dependencies are pre-installed in the environment:
