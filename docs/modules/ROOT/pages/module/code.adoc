...
- **Signature:** Ensures that the function or event signature matches the standard's specification.
- **Return Type:** Verifies that the return types of functions align with the expected types defined in the standard.
- **Parameter Names:** Confirms that the parameter names are consistent with those defined in the standard, enhancing code readability and maintainability.
- **Visibility:** Checks that the visibility (e.g., `external`, `public`) of functions and events adheres to the standard's requirements.
- **Mutability:** For functions, ensures that the mutability (e.g., `view`, `pure`) is correctly specified as per the standard.
