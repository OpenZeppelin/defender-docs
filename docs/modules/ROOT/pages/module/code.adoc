...
|`file-level-reentrancy-model`
|Identifies a possible reentrancy attack vector on a file.
|high
...
|`identify-assembly`
|Identifies assembly code inside a function.
|note
...
|`identify-non-oz-external-imports`
|Identifies external import that is not from the OZ library.
|note
...
|`trust-assumptions`
|Identifies if a function has access control with a modifier.
|introduction
