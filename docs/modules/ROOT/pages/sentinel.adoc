...
Creating a sentinel involves specifying the address to monitor, defining matching rules, and choosing the notification channels.
...
* Transaction has a To, From, or an Event that matches the configured address

* If a *Transaction Condition* is specified

    ** The Transaction matches the *Transaction Condition*.

* If *Events* are selected

    ** The Transaction emits a selected Event and match its *Event Condition* (if any)

* If *Functions* are selected

    ** The Transaction invokes a selected Function and match its *Function Condition* (if any)
...
* *Name* is the name for this slack configuration.  For instance, you might name it after the name of the channel.
