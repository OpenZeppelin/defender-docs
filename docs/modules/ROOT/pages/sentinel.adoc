...
Datadog configurations let Defender forward custom metrics to your Datadog account.  We use the API integration to forward these to Datadog.  For more information about custom metrics, please see https://docs.datadoghq.com/developers/metrics/

Key Notes:
- The metric we send is a COUNT metric, which represents the number of transactions that triggered the sentinel.  
- We do not send zeros, so a lack of data should be expected if the sentinel does not trigger.
- With each metric, we send two tags:  `network` & `sentinel`
