...
[[local-mode]]
=== Local Mode Forta Sentinels

Forta offers the ability to run scanner nodes locally, this can be useful for running only specific detection bots and handling alerts internally instead of publishing to the network. You can monitor scanner nodes running in Local Mode with Defender by creating a new Local Mode Forta Sentinel and specifying the address of your scanner node. After creation your new sentinel will have a Webhook URI, you can add this to your scanner node config file to forward alerts to defender. You can find more information on Local Mode scanner nodes in the https://docs.forta.network/en/latest/scanner-local-mode[Forta Docs,window=_blank].


[[specify-bot-address]]
=== Specifying Forta Detection Bots and Addresses

NOTE: Local Mode Forta Sentinels do not have the option to specify Bot IDs as scanner nodes running in local mode don't support Bot IDs
