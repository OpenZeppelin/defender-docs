...
To avoid overwhelming the receiving webhook with many concurrent requests under a high number of matches, Sentinel sends a JSON object with an `events` property a JSON array containing all the matching events found in a block.
