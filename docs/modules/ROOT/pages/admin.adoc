...
NOTE: The Safe Transaction Service is only available on Mainnet, xDai, BSC, Polygon, Avalanche, Aurora, Optimism, Arbitrum and Rinkeby. Still, you can use Defender Admin on any network; it will just skip syncing with the transaction service if it's not available.
