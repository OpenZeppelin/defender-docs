...
NOTE: At the time of writing, Defender Admin supports OpenZeppelin's Governor contract variant without timelocks. We are working on expanding support for Compound's Governor Alpha and Bravo variants, as well as first class integration with timelocks for all three variants. 
