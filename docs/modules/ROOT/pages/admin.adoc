...
To execute a timelocked proposal, you need a `TimelockController` contract to have enough permissions to execute it. Once proper permissions are in place, just create a proposal as you normally would, ticking the `Timelock` checkbox in the _Execution strategy_ section. Then enter your timelock's address and choose the minimum delay between the proposal's approval and its execution. 
