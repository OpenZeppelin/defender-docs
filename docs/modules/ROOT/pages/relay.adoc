...
For python users there is no official AWS SDK, but it is possible to retrive a JWT token using the https://github.com/capless/warrant#cognito-srp-utility[warrant] library.
...
JWT token will expire in 60 minutes. If you code requires session longer than 60 minutes consider recreating a JWT token or using a https://docs.aws.amazon.com/cognito/latest/developerguide/amazon-cognito-user-pools-using-tokens-with-identity-providers.html[refresh token].
...
==== Send Transaction
To send a transaction to Ethereum blockchain submit a POST request with the desired payload. The payload format is the following:
...
```
==== Query Transaction
To retrive a transaction status and data make a GET request to the `txs` endpoint with the Defender `transactionId`, *not* with the transaction hash.
...
=== Sign Endpoint
To sign an arbitrary data with Relay API make a POST request with a payload containing the hex string to sign. The payload format is:
...
=== Refreshing JWT Token
The JWT token will expire in 60 minutes. If you code requires sessions longer than 60 minutes consider recreating a JWT token or using refresh token.
