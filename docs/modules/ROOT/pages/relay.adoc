...
[[relay-api]]
== Relay API
Relay API exposes two endpoints `txs` and `sign`. The `txs` endpoint allows to send transactions to Ethereum blockchain and query their status. The `sign` endpoint allows to sign arbitrary data with relayer's private key.
...
=== Sign Endpoint
To sign an arbitrary data with Relay API make a POST request with a payload contain hex string. Payload format:
