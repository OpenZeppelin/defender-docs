...
First, make sure you have https://hardhat.org/hardhat-runner/docs/getting-started#installation[Hardhat, window=_blank] installed with ethers v6 (Hardhat version recommended v2.19.0). Follow these steps to create a new directory and project:
...
+
```
forge script script/Deploy.s.sol --ffi --rpc-url https://ethereum-sepolia.publicnode.com -vvvv
```
...
+
```
forge script script/Upgrade.s.sol --ffi --rpc-url https://ethereum-sepolia.publicnode.com -vvvv
```
