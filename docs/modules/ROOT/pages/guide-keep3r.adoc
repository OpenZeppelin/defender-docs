...
If you want to bond KP3R tokens, now is the time to choose how many you want to bond. Note that you can work with zero bonded KP3R tokens, and you can bond more tokens later if you want to do so.

image::guide-keep3r-relayer-bond.png[Bonding your Relayer]

After the transaction to bond your Relayer is sent, your Relayer will now enter *a 3-day waiting period* before it can be activated as a Relayer. You can access your Relayer page at any time to check how much time is remaining.
...
After the 3-day waiting period is over, you will have the option to _Activate as keep3r_. Doing so will send a transaction to activate your Relayer as a Keeper in the network.

image::guide-keep3r-activated.png[Relayer is now a Keeper]

## Create an Autotask

Now that you have your Relayer active as a Keeper, let's set up the Defender Autotask that will instruct the Relay to send them. An Autotask is a snippet of code that runs at regular intervals, in this case, we'll use it to automate the process of bonding, activating, searching for jobs, and executing them.
...
As for the code, paste https://github.com/OpenZeppelin/defender-autotask-examples/blob/master/keep3rs/src/multiple-jobs-keeper.js[this code snippet] from the `defender-autotask-examples` repository. Once you hit save, Defender will start executing the Autotask every minute, taking your Relayer through the bonding and activation process, and then executing available jobs.
...
After working some jobs, you can send your earned KP3R tokens from your Relayer back to your wallet. To do so, go to your Relayer page and choose to _Withdraw funds_. Enter your wallet address, choose KPR as currency, and input the amount of tokens you want to withdraw. Make sure you enter your wallet address correctly, or you may irretrievable lose your tokens!
...
These are some common issues you may see along the way. If you have any questions, don't hesitate to ask on the https://forum.openzeppelin.com/c/support/defender/36[forum]!

### Autotask fails with error "failed to meet quorum"

This error is related to the ethers.js provider being throttled by Alchemy, Infura, or Etherscan due to making too many queries to the network. You can read more about this in the https://docs.ethers.io/v5/api-keys/[ethers.js documentation]. To fix this now, you can apply for an API token as described on the ethers.js docs, and modify the Autotask snippet to use it instead of creating a new `ethers.getDefaultProvider()`. Meanwhile, we are working on adding a custom Defender provider, so you don't need to go through this on your end!
