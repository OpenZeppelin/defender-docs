...
[[network-status]]
=== Status
If a supported network experiences issues, some features in Defender 2.0 for that network may not be working properly. You can check the status of Defender and the supported networks on the https://status.defender.openzeppelin.com/[status page, window=_blank].
