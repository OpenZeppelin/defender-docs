...
The `networks` endpoint is used to retrieve a list of existing networks via a `GET` request.
...
You can query for a specific network types (production or test) by passing the `type` query parameter:
